...
2. Register the client's intake: business type, target public function,
   prior filing history in Bolivia if any.
3. Run the advisor in read-only mode against SICOES (sicoes.gob.bo).
4. Compare the checklist against the client's current documentation
   (NIT status with SIN, SEPREC commercial registration).
...
- named referral relationship with Bolivian-licensed counsel or a
...
a working referral relationship with Bolivian-licensed counsel or a
registered agent for whatever licensed representation the law of
Bolivia requires for actual public-procurement filings.
