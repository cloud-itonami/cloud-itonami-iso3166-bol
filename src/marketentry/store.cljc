(ns marketentry.store
  "SSoT for the Bolivia (BOL) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:on-incumplidos-list?` / `:requires-nit?` / `:nit-verified?` are
  ground truth for the flagship and conditional governor checks."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Fabrication SRL" :portal "sicoes.gob.bo"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-incumplidos-list? false
             :requires-nit? true :nit-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BOL" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "sicoes.gob.bo"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-incumplidos-list? false
             :requires-nit? true :nit-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems SA" :portal "sicoes.gob.bo"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BOL" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export SRL" :portal "sicoes.gob.bo"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-incumplidos-list? true
             :requires-nit? true :nit-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BOL" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics SA" :portal "sicoes.gob.bo"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-incumplidos-list? false
             ...
             :drafted? false :submitted? false
             :jurisdiction "BOL" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech SRL" :portal "sicoes.gob.bo"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? on-incumplidos-list?)          (assoc :engagement/on-incumplidos-list? on-incumplidos-list?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/on-incumplidos-list?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :on-incumplidos-list? (boolean (:engagement/on-incumplidos-list? m))
